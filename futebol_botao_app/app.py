...
def team_badge_html(team: str, size=38):
    team_clean = team.split(" (")[0].strip()
    if team_clean == "Bayern":
        team_clean = "Bayern Munique"
...
def compute_group_tables(state):
    ...
    for group, teams in state["groups"].items():
        stats = {team: {"Equipe":team,"Grupo":group,"Coach":TEAM_META.get("Bayern Munique" if team.split(" (")[0] == "Bayern" else team.split(" (")[0], {}).get("coach", "Unknown"),"Pts":0,"V":0,"E":0,"D":0,"GP":0,"GC":0,"SG":0,"J":0} for team in teams}
