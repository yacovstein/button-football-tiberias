...
def load_state() -> dict:
    ...
    try:
        ...
        assert "groups" in state and "fixtures" in state and "knockout" in state
        return state
    except Exception:
        state = load_initial_state()
        save_state(state)
...
def team_badge_html(display_name: str, size: int = 38) -> str:
...
def get_knockout_structure(qualified: pd.DataFrame, state: dict):
    seed_map = {int(r["Seed"]): r["Equipe"] for _, r in qualified.iterrows()}
    ko = state.get("knockout", {})
    qf = []
    for code, a, b in [("QF1", 1, 8), ("QF2", 2, 7), ("QF3", 3, 6), ("QF4", 4, 5)]:
        item = ko.get(code, {})
        t1 = seed_map.get(a, f"{a}º")
        t2 = seed_map.get(b, f"{b}º")
        g1 = item.get("g1")
        g2 = item.get("g2")
...
def fixture_row(match: dict, idx: int, state: dict):
...
def render_group_table(df: pd.DataFrame, title: str):
    rows_html = ""
...
def render_seed_cards(qualified: pd.DataFrame):
    st.markdown("### Top 8, qualified for knockout stage")
    cols = st.columns(4)
    for i, (_, row) in enumerate(qualified.head(8).iterrows()):
        team = row["Equipe"]
...
def knockout_card(match: dict, state: dict):
